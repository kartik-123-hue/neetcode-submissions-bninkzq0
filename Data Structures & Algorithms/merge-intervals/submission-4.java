class Solution {
    public int[][] merge(int[][] intervals) {
        ...
        //applying sweep line algorithm
        TreeMap<Integer,Integer> map=new TreeMap<>();
        for(int[] interval:intervals){
            //the start of the interval with +1 its going from 0->+ve
            map.put(interval[0],map.getOrDefault(interval[0],0)+1);
            //the end of the interval with -1 its going from +ve->0
            map.put(interval[1],map.getOrDefault(interval[1],0)-1);
        }
        int[] interval=new int[2];
        List<int[]> result=new ArrayList<>();
        int active=0;//indicates the current active interval against that interval
        for(int point:map.keySet()){
            if(active==0){
                //if active ==0 in the start of interval that mean when merged
                //it is the starting point of the new interval
                interval[0]=point;
            }
            active+=map.get(point);
            if(active==0){
                //now if the active reaches 0 this mean it has come from
                //+ve to 0
                //it reaching end interval
                interval[1]=point;
